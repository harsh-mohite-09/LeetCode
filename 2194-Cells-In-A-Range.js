...
var cellsInRange = function (s) {
  let output = [];
  let startLetterCode = s.charCodeAt(0);
  let startNum = +s[1];
  let endLetterCode = s.charCodeAt(3);
  let endNum = +s[4];
  let totalElements =
    (endLetterCode - startLetterCode + 1) * (endNum - startNum + 1);
  for (let i = 0; i < totalElements; i++) {
    if (startNum > endNum) {
      startLetterCode++;
      startNum = +s[1];
    }
    output[i] = `${String.fromCharCode(startLetterCode)}${startNum}`;
    startNum++;
  }
  return output;
};

console.log(cellsInRange("K1:M3"));
